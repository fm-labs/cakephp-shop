...
class StockTransfersController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->StockTransfers->save($stockTransfer)) {
                $this->Flash->success(__('The {0} has been saved.', __('stock transfer')));
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__('The {0} could not be saved. Please, try again.', __('stock transfer')));
            }
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->StockTransfers->delete($stockTransfer)) {
            $this->Flash->success(__('The {0} has been deleted.', __('stock transfer')));
        } else {
            $this->Flash->error(__('The {0} could not be deleted. Please, try again.', __('stock transfer')));
        }
        return $this->redirect(['action' => 'index']);
    }
}
