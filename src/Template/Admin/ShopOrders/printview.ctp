...
<?php echo __("VAT ID: {0}",Configure::read('Shop.Owner.taxId')); ?>
<br />
<br />
    </div>
...
<?php echo __("VAT ID: {0}", $billingAddress['taxid']); ?><br />
<?php endif; ?>
    </div>


    <div class="print_date" style="text-align:right;">
        <?php echo __('Date of order: {0}', $shopOrder->submitted->format("d.m.Y"));?>
        <?php if ($mode == "invoice" && $shopOrder['date_close']):?>
            <br />
            <?php
            echo __('Date of invoice: {0}', date("d.m.Y", strtotime($shopOrder['closed'])));
            ?>
        <?php endif; ?>
        <br />
    </div>
    <div class="print_nr" style="font-weight:bold; font-size: 120%;">
        <?php echo ($mode == "order")
            ? __("Order: {0}", $shopOrder['nr_formatted'])
            : __("Invoice: {0}", $shopOrder['nr_formatted']); ?>
    </div>
    <div class="print_items">
        <table style="width:180mm; margin-bottom: 10mm;" cellpadding="5" cellspacing="0">
            ...
            <tr>
                <td colspan="3" style="text-align:right;font-weight:bold"><?php echo __('Summe exkl. MwSt.'); ?></td>
            ...
            <?php if ($shopOrder['value_coupon'] > 0):?>
                <tr>
                    <td colspan="3" style="text-align:right;"><?php echo __('Gutschein Rabatt'); ?></td>
                    <td style="text-align:right;text-align:right;">- <?php echo $this->Number->currency($shopOrder['value_coupon'], 'EUR'); ?></td>
                </tr>
                <tr>
                    <td colspan="3" style="text-align:right;font-weight:700;"><?php echo __('Endsumme exkl. MwSt.'); ?></td>
                    <td style="text-align:right;font-weight:700;"><?php echo $this->Number->currency($shopOrder['sum'], 'EUR'); ?></td>
                </tr>
            <?php endif;?>
            <tr>
                <td colspan="3" style="text-align:right;"><?php echo __('{0}% MwSt.', 20); ?></td>
                <td style="text-align:right;"><?php echo $this->Number->currency($shopOrder->order_value_tax, 'EUR'); ?></td>
            </tr>
            <tr>
                <td colspan="3" style="text-align:right;font-weight:bold"><?php echo __('Gesamtsumme'); ?></td>
